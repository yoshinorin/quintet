...
test.describe('Article - Redirect', () => {
  test('should be redirect from `/{yyyy}/{mm}/{dd}/` to `/articles/{yyyy}/{mm}/{dd}/` and set collect values in head meta', async ({ page } ) => {

    await page.goto('http://localhost:3000/2024/01/01/standard/');
    await expect(page.url()).toBe('http://localhost:3000/articles/2024/01/01/standard/');
  ...
  test('returns 500 if url not ends with `/{yyyy}/{mm}/{dd}/`: pattern1', async ({ page } ) => {
    const response = await page.goto('http://localhost:3000/aaa/01/01/standard');
    // FIXME: Next.js returns 500
    await expect(response.status()).toBe(404);
  });

  test('returns 500 if url not ends with `/{yyyy}/{mm}/{dd}/`: pattern2', async ({ page } ) => {
    const response = await page.goto('http://localhost:3000/2024/aa/01/standard');
    // FIXME: Next.js returns 500
    await expect(response.status()).toBe(404);
  });

  test('returns 500 if url not ends with `/{yyyy}/{mm}/{dd}/`: pattern3', async ({ page } ) => {
    const response = await page.goto('http://localhost:3000/2024/01/bb/standard');
    // FIXME: Next.js returns 500
    await expect(response.status()).toBe(404);
  });
});
